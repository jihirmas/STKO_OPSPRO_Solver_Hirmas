"""
beam_hinge_command_unassign.py
------------------------------
Command for unassigning any BeamHinge subtype from CAE targets,
...
{
    "name": "unassign_beam_hinge",
    "description": "Unassigns a beam hinge component (BeamEndRelease, BeamRotationalHinge, or BeamShearHinge) from one or more CAE targets. Only targets where this specific hinge is currently assigned are affected; others are silently skipped. Requires an active CAE document. In headless/API mode, 'targets' must be supplied explicitly.",
    "command": "UnassignBeamHinge",
    "inputSchema": {
        "type": "object",
        "properties": {
            "component_id": {
                "type": "integer",
                "description": "ID of the beam hinge component to unassign"
            },
            "targets": {
                "type": "array",
                "description": "List of CAE targets to unassign the hinge from. Required in headless/API mode.",
                "items": {
                    "type": "object",
                    "properties": {
...
class BeamHingeCommandUnassign(AsCommand):
    """
    Unassigns the current BeamHinge from the selected CAE targets.

    Only targets where *this* hinge is actually assigned are processed;
    targets assigned to another hinge (or unassigned) are silently skipped.
    ...
    COMMAND_NAME = 'UnassignBeamHinge'
    ...
    def create(self):
        return BeamHingeCommandUnassign()
    ...
    def execute(self, initial_options: str = ''):
        ...
        if not diff.items:
            self._error = 'Hinge is not assigned to any of the selected targets.'
            print(f'[{self.COMMAND_NAME}] Hinge not assigned to any selected target; aborting.')
            self.terminate(abort=True)
            return

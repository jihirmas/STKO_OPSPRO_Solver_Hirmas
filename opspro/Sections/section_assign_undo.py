"""
section_assign_undo.py
----------------------
Swap-based AsUndoRedoCommand for section assignment and unassignment,
shared by BeamSectionCommandAssign and BeamSectionCommandUnassign.
"""
...
# ---------------------------------------------------------------------------
# SectionAssignUndo  — swap-based undo/redo (mirrors MaterialAssignUndo)
# ---------------------------------------------------------------------------

class SectionAssignUndo(AsUndoRedoCommand):
    """
    Swap-based undo/redo for a section assignment or unassignment.
    ...
    Stores the diff JSON and an *invert* flag.  Each call to execute():
      ...
      2. Returns a new SectionAssignUndo with the opposite direction,
    ...
    def execute(self) -> 'SectionAssignUndo | None':
        App.processEvents()
        try:
            ...
            print(f'[SectionAssignUndo] Error during {direction}: {e}')
            return None
        # Return the inverse command so the next undo/redo is correct
        return SectionAssignUndo(

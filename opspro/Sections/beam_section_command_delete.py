"""
beam_section_command_delete.py
------------------------------
Command for deleting a BeamSection from the document, with undo/redo support.
"""
...
{
    "name": "delete_beam_section",
    "description": "Deletes a beam section from the active document by its ID. Requires an active CAE document; returns status=false if no document is open or if the component_id is not found.",
    "command": "DeleteBeamSection",
    "inputSchema": {
        "type": "object",
        "properties": {
            "component_id": {
                "type": "integer",
                "description": "ID of the beam section to delete"
            }
        },
...
class BeamSectionCommandDelete(AsCommand):
    """
    Command for deleting a BeamSection from the document.

    initial_options (JSON)
    ----------------------
    {
      "component_id": <int>   // ID of the section to delete
    }

    Undo/redo is handled by MpcCaeDocumentGeneralUndo wrapping the return args
    from doc.removePluginCaeComponent().
    """

    COMMAND_NAME = 'DeleteBeamSection'
    ...
    def create(self) -> AsCommand:
        return BeamSectionCommandDelete()
    ...
    def execute(self, initial_options: str = ''):
        ...
        try:
            ...
            section = groups[CAEComponentGroupUIDs.SECTIONS].collection[component_id]
        except Exception as e:
        ...
        self._ret_args = doc.removePluginCaeComponent(section)
        doc.commitChanges()
        doc.dirty = True
